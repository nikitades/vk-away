async function removeFromNewsComments(indicator) {
    const localNewsCommentsInform = newsCommentsInform.bind(null, indicator);
    try {
        ...
        for (; ;) {
            ...
            for (let i in comments.items) {
                ...
                switch (item.type) {
                    case 'topic':
                        await localNewsCommentsInform('Сейчас удаляем: комментарии к темам/беседам...', item.source_id);
                        await remove.myCommentsFromBoard(item.source_id, item.post_id, myId).catch(() => failed = true);
                        break;
                    case 'post':
                        await localNewsCommentsInform('Сейчас удаляем: комментарии в постах на стенах...', item.source_id);
                        await remove.myCommentsFromPost(item.source_id, item.post_id, myId).catch(() => failed = true);
                        break;
                    case 'photo':
                        await localNewsCommentsInform('Сейчас удаляем: комментарии к фотографиям...', item.source_id);
                        await remove.myCommentsFromPhoto(item.source_id, item.post_id, myId).catch(() => failed = true);
                        break;
                    case 'video':
                        await localNewsCommentsInform('Сейчас удаляем: комментарии к видео...', item.source_id);
                        await remove.myCommentsFromVideo(item.source_id, item.post_id, myId).catch(() => failed = true);
                        break;
                    case 'note':
                        await localNewsCommentsInform('Сейчас удаляем: комментарии к заметкам...', item.source_id);
